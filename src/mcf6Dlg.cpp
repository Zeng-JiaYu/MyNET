...
IP_Sum* ip_source = new IP_Sum();						// 对IP计数
...
BOOL Cmcf6Dlg::OnInitDialog()
{
	...
	m_listCtrl.InsertColumn(0,_T("编号"),3,70);                        //1表示右，2表示中，3表示左
	m_listCtrl.InsertColumn(1,_T("时间"),3,170);
	m_listCtrl.InsertColumn(2,_T("长度"),3,72);
	m_listCtrl.InsertColumn(3,_T("源MAC地址"),3,170);
	m_listCtrl.InsertColumn(4,_T("目的MAC地址"),3,170);
	m_listCtrl.InsertColumn(5,_T("协议"),3,70);
	m_listCtrl.InsertColumn(6,_T("源IP地址"),3,175);
	m_listCtrl.InsertColumn(7,_T("目的IP地址"),3,175);
	
	m_listCtrl2.SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);
	m_listCtrl2.InsertColumn(0, _T("编号"), 3, 70);                        //1表示右，2表示中，3表示左
	m_listCtrl2.InsertColumn(1, _T("IP地址"), 3, 170);                        //1表示右，2表示中，3表示左
	m_listCtrl2.InsertColumn(2, _T("数量"), 3, 130);
	...
	*pResult = 0;
	...
	if(CDDS_PREPAINT==pNMCD->nmcd.dwDrawStage)
	{
		*pResult = CDRF_NOTIFYITEMDRAW;
		...
		strcpy_s(buf,local_data->pktType);
...
{	
	...
	strcpy_s(filepath,"SavedData\\");
	strcat_s(filename,thistime);
	strcat_s(filename,".lix");
...
DWORD WINAPI lixsinff_CapThread(LPVOID lpParameter)
{
	...
	time_t local_tv_sec;
	struct pcap_pkthdr *header = NULL;					 // 数据包头
	const u_char *pkt_data=NULL, *pData=NULL;     // 网络中收到的字节流数据
	u_char *ppkt_data = NULL;
	...
	while((res = pcap_next_ex( pthis->adhandle, &header, &pkt_data)) >= 0)
	{
		...
		else if(0x0800 == data->ethh->type) {		//ipv4
			struct  in_addr in;
			in.S_un.S_addr = data->iph->saddr;
			buf = CString(inet_ntoa(in));
		}
		else if(0x86dd ==data->ethh->type ){		//ipv6
			int n;
			for(n=0;n<8;n++)
			{			
				if(n<=6)
		...
		CString num;
		IP_Sum* q = ip_source;
		IP_Sum* p = ip_source->next;
		while (p != NULL) {
			if (p->saddr == buf) {
				p->count++;
				num.Format(_T("%d"), p->count);
				pthis->m_listCtrl2.SetItemText(count-1, 2, num);
				break;
			}
			p = p->next;
			q = q->next;
		...
		if (p == NULL) {
			...
			int nItem2 = pthis->m_listCtrl2.InsertItem(count, num);
			pthis->m_listCtrl2.SetItemText(nItem2, 1, buf);
			buf.Format(_T("%d"), s->count);
			pthis->m_listCtrl2.SetItemText(nItem2, 2, buf);
		}
...
int Cmcf6Dlg::lixsniff_readFile(CString path)
{
	...
	Cmcf6Dlg *pthis =this;						//代码改造自lixsinff_CapThread，为节约工作量，故保留pthis指针
	pcap_t *fp = NULL;
